 ...
 *
 * 描述：
 *         与table_insert_kd类似，但是当插入哈希表后，不能返回一个指向键值的指针。
 *     此函数在次保留以便向后兼容
 *         查看table_insert_kd获得更多信息
 * 返回值：
 *     成功 - TABLE_ERROR_NONE
 *     失败 - Table error code.
 * 参数：
 * table_p - 哈希表指针，指向我们想要插入的一个新的关键字/数据对
 * key_buf - 关键字缓存，如果要按照int排序的话，key_buf必须是(int *).
 * key_size - key_buf缓冲器的大小，如果设置<0，库将作strlen操作并将长度加1，
 *            在后面加上'\0'。如果想对key进行排序，需要将key_size设置为sizeof(int) *
 * data_buf - Buffer of bytes of the data that we are inserting.  If
 * it is NULL then the library will allocate space for the data in the
 * table without copying in any information.  If data_buf is NULL and
 * data_size is 0 then the library will associate a NULL data pointer
 ...
 * be (long **) i.e. the address of a (long *).
 *
 * overwrite - 标志位，如果设置为1，在插入新的数据时，如果数据存在将允许覆盖
 */
extern
