...
#if defined(AIX) || defined(HP_UNIX)
#include <unistd.h>
#else
#include <unistd.h>
#include <malloc.h>
...
static int local_compare(const void *p1, const void *p2,
                         ...
                         int *err_bp)
{
    const table_entry_t * const *ent1_p = (const table_entry_t * const *)p1, * const * ent2_p = (const table_entry_t * const *)p2;
    int   cmp;
    unsigned int  size;
...
{
    const table_linear_t *lin1_p = (table_linear_t *)p1, *lin2_p = (table_linear_t *)p2;
...
{
    unsigned char *inner_p = NULL, *outer_p = NULL;
    int  ret, err_b;
...
static int split(unsigned char *first_p, unsigned char *last_p,
                 ...
                 table_compare_t user_compare, table_t *table_p)
{
    unsigned char *left_p = NULL, *right_p = NULL, *pivot_p = NULL, *left_last_p = NULL, *right_first_p = NULL;
    unsigned char *firsts[MAX_QSORT_SPLITS], *lasts[MAX_QSORT_SPLITS], *pivot = NULL;
     ...
     */
    /*
     * Ludaohong modify for AIX, use malloc instead of alloc,
     * When return, must free pivot, 20040923
     */
    pivot = (unsigned char *)malloc(ele_size);
    if (pivot == NULL)
    {
        /* what else can we do? */
        abort();
                              ...
                              table_mem_free_t free_func, int *error_p)
{
    ...
    table_p->ta_buckets = (table_entry_t **)malloc(size);
    if (table_p->ta_buckets == NULL)
    {
        SET_POINTER(error_p, TABLE_ERROR_ALLOC);
...
int table_clear(table_t *table_p)
{
    ...
    table_entry_t *entry_p = NULL, *next_p = NULL;
                    ...
                    const char overwrite_b)
{
    ...
    table_entry_t *entry_p = NULL, *last_p = NULL, *new_entry_p = NULL;
    void  *key_copy_p = NULL, *data_copy_p = NULL;
    ...
    bucket = hash((unsigned char *)key_buf, ksize, 0) % table_p->ta_bucket_n;
    ...
    if (entry_p != NULL)
    {
        ...
        if (dsize != entry_p->te_data_size)
        {
            ...
            if (table_p->ta_resize_func == NULL)
            {
                ...
                {
                    table_entry_t *tmp_entry_p  = NULL ;

                    tmp_entry_p = (table_entry_t *)realloc(entry_p, new_size);
                    if (tmp_entry_p == NULL)
                    {
                        if (entry_p && new_size != 0)
                        {
                            free(entry_p) ;
                            entry_p = NULL;
                        }
                   ...
                   void **data_buf_p, int *data_size_p)
{
    int  bucket;
    unsigned int ksize;
    table_entry_t *entry_p = NULL, **buckets;
    ...
    {
        table_entry_t *pTmpEntry = NULL;
        pTmpEntry = TABLE_POINTER(table_p, table_entry_t *, entry_p);
        entry_p = pTmpEntry;
        if (entry_p->te_key_size == ksize
            && memcmp(ENTRY_KEY_BUF(entry_p), key_buf, ksize) == 0)
        {
                 ...
                 void **data_buf_p, int *data_size_p)
{
    int  bucket;
    unsigned int ksize;
    unsigned char *data_copy_p = NULL;
    table_entry_t *entry_p = NULL, *last_p = NULL;
...
int table_adjust(table_t *table_p, const int bucket_n)
{
    ...
    (void)printf("growing table to %u\n", buck_n);
...
table_t *table_read(const char *path, int *error_p)
{
    ...
    int  fd, ent_size;
    FILE  *infile = NULL;
    table_entry_t entry, **bucket_p, *entry_p = NULL, *last_p = NULL;
    unsigned long pos;
    table_t *table_p = NULL;
...
int table_write(const table_t *table_p, const char *path, const int mode)
{
    ...
    unsigned long size;
    table_entry_t *entry_p = NULL, **buckets, **bucket_p, *next_p = NULL;
    table_t main_tab;
    FILE  *outfile = NULL;
...
{
    table_entry_t  *entry_p = NULL, **entries, **entries_p;
    table_linear_t linear;
    compare_t  comp_func = NULL;
...
{
    table_entry_t  *entry_p = NULL;
    table_linear_t linear, *linears = NULL, *linears_p = NULL;
    compare_t  comp_func = NULL;
    int   ret;
