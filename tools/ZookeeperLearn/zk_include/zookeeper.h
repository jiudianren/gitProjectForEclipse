...
/*
 * \简要创建一个句柄，用于与zookeeper进行通信。
 *
  此方法创建与该句柄相对应的新句柄和动物园管理员会话。

会话建立是异步的，这意味着直到（并且除非）收到状态为ZOO_CONNECTED_STATE的事件时才认为该会话已建立。


 * \参数主机

逗号分隔主机：端口对，每个对应一个zk
 *服务器。例如“127.0.0.1:3000,127.0.0.1:3001,127.0.0.1:3002”
 * \ param fn
 全球观察者回调函数。当通知被触发时，这个函数将被调用。

 * \ param clientid

 此客户端将重新连接到的以前建立的会话的标识。如果不重新连接到前一个会话，则传递0。
客户可以通过调用\ ref zoo_client_id来访问建立的有效连接的会话标识。

 如果与指定客户端对应的会话已过期，或者由于某种原因客户端无效，则返回的zhandle_t将无效 - zhandle_t状态将指示失败的原因（通常为ZOO_EXPIRED_SESSION_STATE）。


 * \ param上下文

 将与zhandle_t的此实例关联的handback对象。应用程序可以使用\ ref zoo_get_context访问它（例如，在观察者回调中）。该对象在内部不被zookeeper使用，可以为空。

 * \ param flags保留供将来使用。应该设置为零。

 * \返回
 一个指向不透明的zhandle结构的指针。如果它无法创建一个新的zhandle，则该函数返回NULL，并且errno变量指示原因。
...
/**
 * \简短
同步  检查zookeeper中某个节点的存在  。  注意：这里是同步
 *
 *这个功能与\ ref zoo_exists类似，除了它允许指定一个观察者对象而不是布尔型观察标志。
 *
 * \ param zh
 通过调用\ ref zookeeper_init获得的zookeeper句柄
 * \参数路径
节点的名称。
用斜杠分隔节点的祖先的文件名表示。

 * \ param watcher
如果非空，手表将在服务器上的指定znode上设置。
  即使节点不存在，手表也会被设置。这允许客户端观察节点的出现。

 * \ param watcherCtx  用户特定的数据将被传递给观察者回调。
 与由\ ref zookeeper_init设置的全局上下文不同，此观察程序上下文仅与观察程序的给定实例关联。
...
 * ZNONODE节点不存在。
 * ZNOAUTH客户端没有权限。
 * ZBARARGUMENTS - 无效的输入参数
 * ZINVALIDSTATE - zhandle状态为ZOO_SESSION_EXPIRED_STATE或ZOO_AUTH_FAILED_STATE
 * ZMARSHALLINGERROR - 未能发出请求;可能是内存不足
*/
